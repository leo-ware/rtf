...
import Vimeo from "@/components/public-ui/Vimeo"
...
const getThumbnail = async (videoId: string, isYoutube: boolean) => {
    if (isYoutube) {
        const candidates = [
        ...
        const thumbnail = candidates.find((candidate, index) => available[index])
        return { thumbnail }
    } else {
...
const addThumbnails = async (items: VideoCarouselItem[]) => {
    const itemsWithThumbnails = await Promise.all(items.map(async (item) => {
        const thumbnail = await getThumbnail(item.videoId, item.isYoutube ?? false);
        return { ...item, thumbnail: thumbnail.thumbnail };
    }));
    return itemsWithThumbnails;
}

const VideoCarousel = ({ carouselItems }: { carouselItems: VideoCarouselItem[] }) => {
    ...
    return (
        <div className="w-full h-fit">
            <div className="hidden md:block z-0 relative w-full h-fit overflow-x-hidden">
                                ...
                                <div className="absolute flex justify-end w-full h-full">
                                    ...
                                    >
                                        <div className="w-full h-full flex items-center justify-end">
                                            <div className="text-white text-right font-serif text-xl lg:text-4xl">
                                                {item.title}
                                            </div>
                                        </div>
                                    </div>
