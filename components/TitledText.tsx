...
const TitledText = ({ title, children, color = "pewter", className }: { title: string, children: string, color?: string, className?: string }) => {
    return (
        <div className={cn("mx-auto w-9/12 md:w-8/12 h-fit flex flex-col items-center justify-center gap-4", className)}>
            <Header color={color} className="text-center underline">
                {title}
            </Header>
